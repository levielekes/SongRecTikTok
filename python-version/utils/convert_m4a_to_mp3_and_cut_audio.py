...
def apply_audio_delay(input_file, output_file, delay_ms):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(0.2)
            AudioSegment.from_mp3(output_file)
            ...
            logger.error(f"Error applying audio delay: {ex}")
            if attempt < max_retries - 1:
                logger.info(f"Retrying audio delay attempt {attempt+1}/{max_retries}")
            else:
                logger.error(f"Skipping file {destination_path} due to failed audio delay attempts")
                return False
...
for filename in os.listdir(env_config.download_dir):
    ...
    try:
        ...
        logger.error(f"Error loading audio file {destination_path}: {str(e)}", exc_info=True)
        continue
